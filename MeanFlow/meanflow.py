...
class MeanFlow:
    ...
    def loss(self, model, clean_img, noised_img, c=None):
        ...
        clean_img = self.normer.norm(clean_img)
        noised_img = self.normer.norm(noised_img)

        z = (1 - t_) * clean_img + t_ * noised_img
        v = noised_img - clean_img
    ...
    @torch.no_grad()
    def simulate(self, model, noised_img, c,
                          sample_steps=5, device='cuda'):
        model.eval()

        # noised_img: (bs, c, h, w), c: (bs, ). c is the correspond labels of noised_img
        noised_img = self.normer.norm(noised_img)
        ...
        for i in range(sample_steps):
            t = torch.full((noised_img.size(0),), t_vals[i], device=device)
            r = torch.full((noised_img.size(0),), t_vals[i + 1], device=device)
            ...
            v = model(noised_img, t, r, c)
            noised_img = noised_img - (t_-r_) * v

        denoised_img = self.normer.unnorm(noised_img)
        return denoised_img
